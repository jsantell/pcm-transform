 ...
 * @option {Number} samplesPerValue [required]
 * @option {Boolean} json
...
function PCMTransform (options) {
  ...
  if (typeof options.samplesPerValue !== "number" || options.samplesPerValue <= 0) {
    throw new Error("samplesPerValue must be a number greater than 0.");
  }

  this.samplesPerValue = options.samplesPerValue;
  ...
  // later
  this._bytesPerValue = this.samplesPerValue * this.inputChannels * (this.inputBitDepth / 8);
...
PCMTransform.prototype._valueToBuffer = function (num) {
  var buffer = new Buffer(this.inputBitDepth / 8);
  buffer["writeInt" + this.inputBitDepth + (this.inputBitDepth === 8 ? "" : "LE")](num, 0);
  return buffer;
};
